...
// Linear interpolation helper (put this somewhere global or pass as lambda)
double interp_prior(const vector<double>& periods,
                    const vector<double>& pdf,
                    double P,
                    double floor) // ← adjustable floor value
{
    if (P <= periods.front() || P >= periods.back())
        return floor; // soft edge

    auto it = std::upper_bound(periods.begin(), periods.end(), P);
    size_t idx = std::distance(periods.begin(), it) - 1;

    double x0 = periods[idx];
    double x1 = periods[idx+1];
    double y0 = pdf[idx];
    double y1 = pdf[idx+1];

    double t = (P - x0) / (x1 - x0);
    double interp = y0 + t * (y1 - y0);

    return std::max(interp, floor); // ← enforce floor everywhere
}
...
double rv_curve_binary(double x, double amplitude, double period, double offset, double phase,
                       double amplitude_two, double period_two, double phase_two) {
    // The general form of the sinusoid is:
    // y(x) = amplitude * sin(2 * M_PI * (x / period + phase)) + offset

    double result = amplitude * sin(2 * M_PI * (x / period + phase)) + offset;
    result += amplitude_two * sin(2 * M_PI * (x / period_two + phase_two));
    return result;
}
...
vector<double> genOptimalPeriodogramSamples(const vector<double>& t, double sample_factor, double forced_min_p, double forced_max_p, int forced_N){
    ...
    if (not forced_N){
        ...
        if (!isfinite(df)) {
            cout << "[WARN] Periodogram sampling generator: df is nan, using generic 100000 samples." << endl;
            Npoints = 100000;
            df = (1/min_p-1/max_p)/Npoints;
        }
    }
    else{
